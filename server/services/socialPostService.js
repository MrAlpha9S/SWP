        ...
        const result = await pool.request()
            ...
            .query(`
                ...
                WHERE sc.category_tag = @categoryTag
                  AND sp.is_pending = 0
                GROUP BY sp.title, sp.content, sp.created_at,
...
const AddLike = async (auth0_id, post_id = null, comment_id = null, created_at) => {
    try {
        ...
        const result = await pool.request()
            ...
            .input('created_at', sql.DateTime, created_at)
            .query(`INSERT INTO [social_likes] ([user_id], [post_id], [comment_id], [created_at])
                    VALUES
...
const getPostOwnerInfoByCommentId = async (comment_id) => {
    ...
    const query = `
        SELECT postOwner.auth0_id AS post_owner_auth0_id,
