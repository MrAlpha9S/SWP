...
const postUserProfile = async (userAuth0Id,
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                INSERT INTO user_profiles (user_id, readiness, start_date, quit_date, expected_quit_date,
...
const getUserProfile = async (userAuth0Id) => {
    const profileExists = await userProfileExists(userAuth0Id);
    if (!profileExists) {
        try {
            const pool = await poolPromise;
            const userId = await getUserIdFromAuth0Id(userAuth0Id);
...
module.exports = {userProfileExists, postUserProfile, getUserProfile}