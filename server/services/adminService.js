...
const rejectCoach = async (id) => {
  ...
  try {
    ...
    // Xóa dữ liệu từ bảng coach_info
    await transaction.request()
      .input('id', sql.Int, id)
      .query('DELETE FROM coach_info WHERE coach_id = @id');
...
const getPendingCoachDetails = async (id) => {
  const pool = await poolPromise;
  const result = await pool.request()
    .input('id', sql.Int, id)
    .query(`
      ...
      FROM users u
      LEFT JOIN coach_info ci ON u.user_id = ci.coach_id
      WHERE u.user_id = @id AND u.is_pending_for_coach = 1
    `);
  return result.recordset[0];
};
