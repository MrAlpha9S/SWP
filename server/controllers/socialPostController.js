...
const handleDeleteComment = async (req, res) => {
    ...
    if (!comment_id) {
        return res.status(400).json({
            success: false,
            message: 'error in handleDeleteComment: params is required',
            data: null
        });
    }
...
const handleApprovePost = async (req, res) => {
    ...
    if (!post_id) {
        return res.status(400).json({
            success: false,
            message: 'error in handleApprovePost: params is required',
            data: null
        });
    }
...
const handlePostSocialPosts = async (req, res) => {
    ...
    if (!category_id || !auth0_id || !title || !content || !created_at) {
        return res.status(400).json({
            success: false,
            message: 'error in handlePostSocialPosts: params is required',
            data: null
        });
    }
...
const handleUpdateSocialPosts = async (req, res) => {
    ...
    if (!category_id || !post_id || !title || !content || !created_at) {
        return res.status(400).json({
            success: false,
            message: 'error in handleUpdateSocialPosts: params is required',
            data: null
        });
    }
...
const handleDeleteSocialPosts = async (req, res) => {
    ...
    if (!post_id) {
        return res.status(400).json({
            success: false,
            message: 'error in handleDeleteSocialPosts: params is required',
            data: null
        });
    }
...
const handleAddComment = async (req, res) => {
    ...
    if (!auth0_id || !post_id || !content || !created_at || is_reported === undefined) {
        return res.status(400).json({
            success: false,
            message: 'error in handleAddComment: params is required',
            data: null
        });
    }
    ...
    try {
        ...
        console.log(add);
        await processAchievementsWithNotifications(auth0_id);
        const comment = await getPostOwnerInfoByCommentId(add)
        const metadata = {
            inner_type: 'post-reply',
            category_tag: comment.category_tag,
            post_id: post_id,
            comment_id: add,
        }
        await createNotificationService(comment.post_owner_auth0_id, `Người dùng ${comment.commenter_username} vừa bình luận bài viết ${comment.post_title} của bạn.`, `Bình luận: ${content}`, 'community', metadata)
        const io = socket.getIo()
        io.to(`${comment.post_owner_auth0_id}`).emit('reply', {
            ...metadata,
            commenter_auth0_id:  comment.commenter_auth0_id,
...
const handleAddLike = async (req, res) => {
    ...
    if (!auth0_id || !created_at === undefined) {
        return res.status(400).json({
            success: false,
            message: 'error in handleAddLike: params is required',
            data: null
        });
    }
...
module.exports = {
    handleDeleteComment,
    handleApprovePost,
    handleGetIsPendingPosts,
    handleDeleteSocialPosts,
    handleUpdateSocialPosts,
    getPostAndCommentCount,
    handleGetPostByCategory,
    handleGetPosts,
    handleGetPostComments,
    handlePostSocialPosts,
    handleAddComment,
    handleAddLike
};
